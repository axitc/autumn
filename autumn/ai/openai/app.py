...
vector_store = Milvus(
        ...
        # index_params = {'index_type':'FLAT', 'metric_type':'L2'}, # idk what it does. not docs either :(
        auto_id = True,
        )
...
def upsert(request, summary, tag):
    ...
    # ^ used page_text instead of text cuz it was reserved
    vector_store.upsert(documents=[doc], ids=vector_store.get_pks(f"url in ['{request.url}']"))
    # ^ id didnt accept url string. so had to match with url to get primary key
