...
vector_store = Milvus(
        ...
        # index_params = {'index_type':'FLAT', 'metric_type':'L2'}, # idk what it does. no docs either :(
        auto_id = True,
        )
...
def summarize(title,text):
    pretext = preprocess(text, 100, 1000)
...
def upsert(request, summary, tag):
    ...
    # ^ page_text is used instead of text cuz its reserved
    vector_store.upsert(documents=[doc], ids=vector_store.get_pks(f"url in ['{request.url}']"))
    # ^ id didnt accept url string. so have to match url in meta to get primary keys
